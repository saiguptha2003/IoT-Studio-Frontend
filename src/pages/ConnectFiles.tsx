...
const ConnectFiles: React.FC = () => {
  ...
  const fetchFiles = async () => {
    try {
      const response = await fetch('http://localhost:5001/services/WareHouse/getConnectFilesList', {
        headers: getHeaders(),
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await fetch(`http://localhost:5001/services/WareHouse/deleteConnectFile/${fileToDelete}`, {
        method: 'DELETE',
        headers: getHeaders(),
      });
      await fetchFiles();
  ...
  const handleDownload = async (id: string) => {
    try {
      const response = await fetch(`http://localhost:5001/services/WareHouse/getConnectFile/${id}`, {
        headers: getHeaders(),
      });
